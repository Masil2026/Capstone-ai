...
| 2. 의도 파악 + 스트리밍 | Orchestrator (GPT-4.1) | 도구 호출·외부 API 수집·텍스트 스트리밍 |
| 3. 데이터 전처리 | preprocessor_agent (GPT-4o-mini) | Tavily 비정형 결과 요약 (도구 함수 내부) |
| 4. 타입 판별 | classification_agent (GPT-4o-mini) | 스트리밍 완료 후 응답 의도 분류 |
| 5. 임베딩 생성 | OpenAI Embeddings | 사용자·AI 메시지 벡터화 |
| 6. done 이벤트 전송 | FastAPI SSE | 최종 구조화 페이로드 전달 |
...
## 3. 비정형 데이터 전처리 — preprocessor_agent
...
스트리밍 완료 후 사용자 메시지와 AI 응답 전문에 대한 임베딩 벡터를 생성합니다.

| 항목 | 내용 |
|------|------|
| 모델 | `text-embedding-3-small` (OpenAI) |
| 차원 | `1536` |
| 생성 대상 | `userMessage.content`, `assistantMessage.content` |
| 용도 | Spring Boot가 `chat_messages.embedding` 컬럼에 저장. 향후 벡터 유사도 검색에 활용 |
...
[0] X-Internal-Token 검증
  ↓
[1] memory 동기화 + chat_history 로드 (Redis)
  ↓
[2] orchestrator_agent.run_stream()
  ├─ 필요시 도구 호출 (search_web → preprocessor_agent 내부 호출)
  └─ 텍스트 토큰 → event: chunk 반복 전송
  ↓
[3] 스트리밍 완료 → 전체 응답 텍스트 확보
  ↓
[4] classification_agent.run() → ResponseClassification 추출
  ↓
[5] 임베딩 생성 (user + assistant 각 1회)
  ↓
[6] memory 갱신 판단 → Redis 업데이트 (변경 있을 때만)
  ↓
[7] chat_history 저장 (Redis, 최대 20개)
  ↓
[8] done 페이로드 구성 → event: done 전송
```
