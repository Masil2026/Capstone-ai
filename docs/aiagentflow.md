...
| 단계 | 주요 컴포넌트 | 역할 |
|------|-------------|------|
| 1. 의도 파악 | Orchestrator (GPT-4.1) | 사용자 입력 분석 및 도구 선택 |
...
| 5. 최종 생성 | GPT-4.1 | 컨텍스트를 반영한 최종 답변 / 일정 구성 |

---

## 1. 입력 및 의도 파악 — Orchestrator (GPT-4.1)

사용자가 채팅 UI를 통해 텍스트나 이미지를 입력하면 **GPT-4.1** 기반의 AI 에이전트가 가동됩니다.
...
## 4. 최종 응답 생성 및 저장 — GPT-4.1

전처리가 완료된 정형 + 비정형 데이터가 메인 모델로 전달됩니다.

- **최종 일정 생성**: GPT-4.1이 전처리된 데이터 + Redis 단기 히스토리 + 장기 요약(ai_summary) + 사용자 취향(preferences)을 결합하여 최종 응답(일정)을 생성
