...
def _build_synthesizer_prompt(d: SynthesizerDeps) -> str:
    ...
    lines = [
        ...
        "- `ai_summary`: 이전 대화 요약(## 이전 대화 요약)이 있으면 그 내용과 이번 일정 생성 결과를 합쳐 전체 흐름을 하나의 요약으로 작성한다.",
        "  예) '제주도 3박 4일 일정 생성. 5월 1일~3일. 성인 2명. 첫날 저녁 해산물 식사 포함.'",
        "- `preferences`: 아래 [## preferences 추출 규칙] 참고. 반드시 작성할 것.",
        "",
        "## preferences 추출 규칙",
        "사용자 메시지와 이번 대화 전체에서 취향을 적극 추출하여 dict로 반환한다.",
        "가볍게 지나가는 말(~먹고 싶어, ~가 좋아, ~는 싫어)도 모두 취향으로 간주한다.",
        "",
        "추출 대상 카테고리 (키 예시):",
        "- food: 음식 종류·식재료 (예: ['해산물', '현지 음식'])",
        "- food_avoid: 피하고 싶은 음식 (예: '고수')",
        "- transport: 이동 수단 선호 (예: '대중교통')",
        "- accommodation: 숙박 스타일 (예: '호텔')",
        "- activities: 관심 활동 (예: ['자연', '해수욕', '문화재'])",
        "- pace: 여행 속도 (예: '여유롭게')",
        "- budget_style: 예산 방식 (예: '가성비')",
        "- 그 외 취향도 적절한 키로 추가한다.",
        "",
        "기존 ## 사용자 취향이 있으면 그 내용을 포함한 전체 dict를 반환한다.",
        "취향 관련 내용이 전혀 없고 기존 취향도 없는 경우 빈 dict {}를 반환한다.",
        '출력 예시: {"food": ["해산물", "현지 음식"], "transport": "대중교통", "activities": ["자연", "해수욕"]}',
        "",
        "## day_plans 각 항목 형식",
