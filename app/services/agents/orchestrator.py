...
- itinerary·change 처리 후에는 `ai_summary` 필드에 반드시 작성한다.
  이전 대화 요약(## 이전 대화 요약)이 있으면 그 내용과 이번 대화를 합쳐 전체 흐름을 하나의 요약으로 다시 작성한다.
...
### preferences — 반드시 적극적으로 추출할 것
**사용자 메시지에서 아래 항목에 해당하는 내용이 조금이라도 언급되면 무조건 preferences에 기록한다.**
아주 가볍게 지나가는 말("~먹고 싶어", "~가 좋아", "~는 싫어")도 모두 취향으로 간주한다.

추출 대상 카테고리 (키 예시):
- `food` : 음식 종류·식재료 선호 (예: "seafood", "현지 음식", "한식", "채식")
- `food_avoid` : 피하고 싶은 음식 (예: "고수", "돼지고기")
- `transport` : 이동 수단 선호 (예: "대중교통", "택시", "도보")
- `accommodation` : 숙박 스타일 (예: "호텔", "게스트하우스", "풀빌라")
- `activities` : 관심 활동 (예: ["자연", "해수욕", "문화재", "쇼핑"])
- `pace` : 여행 속도 (예: "여유롭게", "빡빡하게")
- `budget_style` : 예산 사용 방식 (예: "가성비", "프리미엄")
- `travel_with` : 동행 특성 (예: "어린이 동반", "부부", "친구")
- 위 목록 외에도 취향으로 보이면 적절한 키를 만들어 추가한다.
...
취향 관련 내용이 전혀 없고 기존 취향도 없는 경우 빈 dict {}를 반환한다."""
